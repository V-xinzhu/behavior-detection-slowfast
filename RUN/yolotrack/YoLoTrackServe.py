...
from utils.Format_conversion import DeepSortLikeDection
...
import time
import cv2
logs=get_logger("yolotrack", filename="YoloTrackServe")
...
def process_one_clip(system, id_to_ava_labels, cfg,data_id):
    ...
    yolo=cfg.get('yolo')
    yolo_iou=yolo.get('iou_threshold',)
    yolo_conf=yolo.get("confidence_threshold")
# 1. 连接Redis并取出字节
    tt=time.time()
    ...
    logs.info(f"🛢️ extracting from redis耗时:{1000*(time.time()-tt)}ms")
#yolo检测
    tt=time.time()
    res=system.yolo_model.track(imgs_data,verbose=False,conf=yolo_conf,iou=yolo_iou)
    yolo_preds=DeepSortLikeDection(res)
    logs.info(f"📷 processing YOLOtrack耗时：{1000*(time.time()-tt)}")
    # 行为识别 slowfast
    tt=time.time()
...
# 如果至少有一个人
    ...
    Data = build_final_data(yolo_preds.pred[FPS-1], id_to_ava_labels,pic)
    logs.info(f"📡 processing export json耗时：{1000*(time.time()-tt)}")
    return Data
class ResultHandler_track(BaseHTTPRequestHandler):
